...
class Fd(list):
    ...
    *Fd* is also defined by the tensor product of
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in args:
            if isinstance(F, (S, PP)):
                ...
                raise TypeError("Unknown function space")
    ...
    @property
    def a(self):
        return [(F.name, F.a) for F in self]
...
class PP(Fd):
    ...
    def PP2S(self):
        ...
        if n + k != len(t):
            raise RuntimeError("PP2S: dims mismatch")
        return S(k, t)
...
class S(Fd):
    ...
    def __init__(self, k, t, name=''):
        ...
        if self.n <= 0:
            raise ValueError("invalid knot sequence")

    @property
    def n(self):
        return len(self.t) - self.k
	...
	""" Knot optimal placement based on SPLOPT (see chapter XIII Carl de Boor's book) """
	k, n = self.k, self.n
	if n != len(tau):
...
class pp(fd, PP):
    ...
    def inPP(cls, Pkxi):
        ...
        if not isinstance(Pkxi, PP):
            raise TypeError("non-PP obj in input")
    ...
    def cmpcoef(self, tau=None, gtau=None, dgtau1=0., dgtauN=0.,
        ...
        if l > 1:
            if k != 4:
                raise ValueError("4th order required!")
            n = np.int32(l + 1)
            ...
            if method == "newton":
                if m == 1:
                    coef = ppk.newton_coef_1d(n=k, x=np.float64(tau),
                                              f=np.float64(gtau))
                else:
                    coef = ppk.newton_coef_nd(n=k, m=m, x=np.float64(tau),
                                              f=np.float64(gtau))
            elif method == "chebyshev":
                if m == 1:
                    coef, xmin, xmax = pch.chebyshev_coef_1d(
                        nd=k, xd=np.float64(tau), yd=np.float64(gtau)
                    )
                else:
                    raise ValueError("ND chebyshev not available yet")
            else:
                raise ValueError("unknown input argument: method")
...
class s(fd, S):
    ...
    def inS(cls, Skt):
        ...
        if not isinstance(Skt, S):
            raise TypeError("non-S obj in input")
    ...
    def cmpcoef(self, tau, gtau, VDS=False):
        ...
        if ltau != self.n:
            raise ValueError("data points are not n")
        if lgtau % ltau != 0:
            raise ValueError("tau/gtau data mismatch")
        if not VDS:
            ...
            if m == 1:
                ...
                work, q, self.coef, iflag = ppk.spli2d(n=n, k=k, m=m,
                                                       tau=np.float64(tau),
                                                       gtau=np.float64(gtau),
                                                       t=np.float64(self.t))
        else:
            ...
            self.coef = gtau
        if iflag != 1:
            raise RuntimeError("SPLINT/SPLI2D failed")
        # tau, gtau and coef must change at the same time
        self._tau, self._gtau = tau, gtau
    ...
    def s2pp(self):
        ...
        scrtch, breaks, coef, l = ppk.bsplpp(k=k, n=n, t=np.float64(self.t),
                                             bcoef=np.float64(self.coef))
        if sum(breaks[l+1:]) > 0:
