! divdif from http://people.sc.fsu.edu/~jburkardt/f_src/divdif/divdif.html
! original source have been modified to comply with C-wrapping by f2py for
! use in python
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: ntab

  real ( kind = 8 ), intent(out) :: diftab(ntab)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  logical r8vec_distinct
  real ( kind = 8 ), intent(in) :: xtab(ntab)
  real ( kind = 8 ), intent(in) :: ytab(ntab)
...
!
!    Output, integer ( kind = 4 ) NA, the size of the difference table for the
...
!
  ...
  real ( kind = 8 ) x0(nd)
  real ( kind = 8 ), intent(out) :: xa(nd+1)
  real ( kind = 8 ), intent(in) :: xd(nd)
  real ( kind = 8 ) y0(nd)
  real ( kind = 8 ), intent(out) :: ya(nd+1)
...
!
  ...
  real ( kind = 8 ), intent(in) :: diftab(ntab)
  real ( kind = 8 ), intent(out) :: diftab2(ntab+1)
  integer ( kind = 4 ) i
  integer ( kind = 4 ), intent(out) :: ntab2
  real ( kind = 8 ), intent(in) :: xtab(ntab)
  real ( kind = 8 ), intent(out) :: xtab2(ntab+1)
...
!    L(I,NTAB,XTAB)(X) is a polynomial of degree NTAB-1 which is zero at
...
!
  ...
  real ( kind = 8 ), intent(out) :: diftab(ntab,ntab)
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(in) :: xtab(ntab)
!
...
!
!    Input, real ( kind = 8 ) XD(ND), the X values upon which the 
!    Lagrange basis polynomials are to be based.
!
!    Output, real ( kind = 8 ) XDP(ND-1), the X values upon with
!    the derivative difference table is based.  In fact, these are
!    all 0.
!
!    Output, real ( kind = 8 ) DDP(ND-1,ND), the divided difference 
...
!
  ...
  real ( kind = 8 ) dd(nd)
  real ( kind = 8 ), intent(out) :: ddp(nd-1,nd)
  integer ( kind = 4 ) j
  real ( kind = 8 ), intent(in) :: xd(nd)
  real ( kind = 8 ), intent(out) :: xdp(nd-1)
  real ( kind = 8 ) yd(nd)
!
...
!    Lagrange basis polynomials are to be based.
!
!    Input, integer ( kind = 4 ) K, the index of the derivative.
!
!    Output, real ( kind = 8 ) XDP(ND-K), the X values upon with
!    the derivative difference table is based.  In fact, these are
!    all 0.
!
!    Output, real ( kind = 8 ) DDP(ND-K,ND), the divided difference 
!    tables for all the Lagrange basis polynomials.  Column J of DDP
!    contains the table for basis polynomial associated with XD(J).
!
  implicit none
  ...
  real ( kind = 8 ) dd(nd)
  real ( kind = 8 ), intent(out) :: ddp(nd-k,nd)
  integer ( kind = 4 ) j
  real ( kind = 8 ), intent(in) :: xd(nd)
  real ( kind = 8 ), intent(out) :: xdp(nd-k)
  real ( kind = 8 ) yd(nd)
!
!  Process the vectors one column at a time.
!
...
!
!    Output, real ( kind = 8 ) XDP(ND-1), the abscissas for the divided
!    difference table for the derivative.
!
!    Output, real ( kind = 8 ) YDP(ND-1), the divided difference 
...
!
  ...
  real ( kind = 8 ), intent(out) :: ydp(nd-1)
!
...
!    difference table.
!
!    Input, real ( kind = 8 ) DD(ND), the divided difference table.
!
!    Input, integer ( kind = 4 ) K, the index of the derivative.
!    0 <= K
!
!    Output, real ( kind = 8 ) XDK(ND-K), the abscissas for the divided
!    difference table for the derivative.
!
!    Output, real ( kind = 8 ) DDK(ND-K), the divided difference 
!    table for the derivative.
!
  implicit none
  ...
  real ( kind = 8 ), intent(in) :: dd(nd)
  real ( kind = 8 ) dd_temp(nd)
  real ( kind = 8 ), intent(out) :: ddk(nd-k)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  integer ( kind = 4 ) ndk
  real ( kind = 8 ), intent(in) :: xd(nd)
  real ( kind = 8 ) xd_temp(nd)
  real ( kind = 8 ), intent(out) :: xdk(nd-k)
...
!
  ...
  call dif_shift_zero ( ndk, xd_temp, dd_temp )
!
...
!
  do j = 1, k
    ...
    do i = 1, ndk
      dd_temp(i) = real ( i, kind = 8 ) * dd_temp(i+1)
    end do
...
!
  ...
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(inout) :: xd(nd)
  real ( kind = 8 ), intent(in) :: xv
  real ( kind = 8 ), intent(inout) :: yd(nd)
!
...
!
  ...
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(in) :: xd(nd)
  real ( kind = 8 ), intent(in) :: xv(nv)
  real ( kind = 8 ), intent(in) :: yd(nd)
  real ( kind = 8 ), intent(out) :: yv(nv)
...
!
  ...
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(out) :: weight(n)
  real ( kind = 8 ), intent(in) :: xtab(n)
  real ( kind = 8 ) yvala
  real ( kind = 8 ) yvalb
...
!
  ...
  real ( kind = 8 ) b
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(out) :: weight(norder)
  real ( kind = 8 ), intent(out) :: xtab(norder)
!
...
!
  ...
  real ( kind = 8 ), intent(in) :: poly_cof(n)
  real ( kind = 8 ), intent(out) :: poly_cof2(n+1)
  integer ( kind = 4 ) i
!
...
!
  ...
  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(out) :: poly_cof(ntab,ntab)
  real ( kind = 8 ), intent(in) :: xtab(ntab)
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: ival
  real ( kind = 8 ), intent(out) :: poly_cof(ntab)
  real ( kind = 8 ), intent(in) :: xtab(ntab)
!
!  Check IVAL.
!
...
!
  ...
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  real ( kind = 8 ), intent(inout) :: poly_cof(n)
  real ( kind = 8 ), intent(in) :: scale, shift
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nroots

  real ( kind = 8 ), intent(out) :: diftab(nroots+1)
  integer ( kind = 4 ), intent(out) :: ntab
  real ( kind = 8 ), intent(in) :: roots(nroots)
  real ( kind = 8 ), intent(out) :: xtab(nroots+1)

  ntab = nroots + 1
!
!  Build the appropriate difference table for the polynomial
!  through ( ROOTS(I), 0 ) of degree NTAB-1.
!
  diftab(1:ntab-1) = 0.0D+00
!
!  Append the extra data to make a monic polynomial of degree NTAB
...
!
  ...
  real ( kind = 8 ), intent(out) :: c(nroots+1)
