...
! - date: March 2017
! - source: //people.sc.fsu.edu/~jburkardt/f_src/pppack/pppack.html
!
! Original source code has been modified mainly to avoid common data block,
! plus other style and performance changes (goto removal, code refactoring).
! Tested against the *_prb problems available online.
!
!*****************************************************************************80

module ppcolloc_data
  implicit none
  integer ( kind = 4 ), parameter :: npiece = 100
  integer ( kind = 4 ), parameter :: ncoef = 2000
  ! old common approx
  integer ( kind = 4 ) :: l, kpm
  real ( kind = 8 ), dimension(npiece) :: breaks
  real ( kind = 8 ), dimension(ncoef) :: coef
  save
end module ppcolloc_data
module colloc_data
  implicit none
  ! old common other
  integer ( kind = 4 ) :: itermx, k
  real ( kind = 8 ), dimension(19) :: rho
  ! old common side
  integer ( kind = 4 ) :: m, iside
  real ( kind = 8 ), dimension(20) :: xside
  save
end module colloc_data
module l2approx_data
  implicit none
  integer ( kind = 4 ), parameter :: ltkmax = 2000
  integer ( kind = 4 ), parameter :: lpkmax = 100
  ! old common approx
  integer ( kind = 4 ) :: l, k
  real ( kind = 8 ), dimension(lpkmax) :: breaks
  real ( kind = 8 ), dimension(ltkmax) :: coef
...
module l2ir_data
  ...
  integer ( kind = 4 ) :: ntau
  real ( kind = 8 ) :: totalw
  real ( kind = 8 ), dimension(ntmax) :: tau, gtau, weight
  save
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nrow, nroww
  ...
  integer ( kind = 4 ), intent(in) :: nbandl, nbandu
  real ( kind = 8 ) pivot
  real ( kind = 8 ), intent(inout) :: w(nroww,nrow)
...
!
  ...
  if ( nrow == 1 ) then
    iflag = transfer( w(middle,nrow) == 0.0D+00 , iflag ) + 1
    return
  end if
...
!
  if ( nbandl <= 0 ) then
    ...
    if ( any(  w(middle,1:nrow-1) == 0.0D+00 ) ) then
      iflag = 2
      return
    end if
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nrow, nroww, nbandl, nbandu
  real ( kind = 8 ), intent(in) :: w(nroww,nrow)
  real ( kind = 8 ), intent(inout) :: b(nrow)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  integer ( kind = 4 ) jmax
  integer ( kind = 4 ) middle
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nbands, nrow
  real ( kind = 8 ), intent(inout) :: w(nbands,nrow)
  real ( kind = 8 ), intent(out) :: diag(nrow)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nbands, nrow
  real ( kind = 8 ), intent(in) :: w(nbands,nrow)
  real ( kind = 8 ), intent(inout) :: b(nrow)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, left, nderiv

  real ( kind = 8 ), intent(out) :: a(k,k)
  ...
  real ( kind = 8 ), intent(in) :: x, t(left+k)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, m, n

  real ( kind = 8 ), intent(in) :: bcoef(n,m), t(n+k)
  real ( kind = 8 ) biatx(k)
  real ( kind = 8 ), intent(out) :: breaks(*)
  real ( kind = 8 ), intent(out) :: coef(m,k,*)
  real ( kind = 8 ) diff
  real ( kind = 8 ) fkmj
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
  ...
  integer ( kind = 4 ) lsofar
  integer ( kind = 4 ) mm
  real ( kind = 8 ), intent(out) :: scrtch(k,k,m)
...
!
    ...
    if ( k <= 1 ) then
      ...
      ! then cycle to next value of left
...
!
      ...
      do jp1 = 2, k
        ...
        do mm = 1, m
          ...
          coef(mm,kmj,lsofar) = dot_product(biatx(1:jp1), scrtch(1:jp1,kmj,mm))
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, n, jderiv

  real ( kind = 8 ) aj(k)
  real ( kind = 8 ), intent(in) :: bcoef(n), t(n+k), x
  real ( kind = 8 ) bvalue
  real ( kind = 8 ) dl(k)
  real ( kind = 8 ) dr(k)
  integer ( kind = 4 ) i
...
!f2py integer, depend(bcoef) :: n = shape(bcoef,0)
!f2py integer, check(n*m == shape(bcoef,0)*shape(bcoef,1)), depend(bcoef,n) :: m = shape(bcoef,1)
  integer ( kind = 4 ), intent(in) :: k, n, m, jderiv

  real ( kind = 8 ) aj(k,m)
!f2py real ( kind = 8 ), intent(in), dimension(*,*) :: bcoef
  real ( kind = 8 ), intent(in) :: bcoef(n,m)
  real ( kind = 8 ), intent(in) :: t(n+k), x
  real ( kind = 8 ), intent(out) :: bvalue(m)
  real ( kind = 8 ) dl(k)
  real ( kind = 8 ) dr(k)
  real ( kind = 8 ) fact
  integer ( kind = 4 ) i
...
!
    do j = 1, jderiv
      ...
      ilo = k - j
      do jj = 1, k - j
        fact = real ( k - j, kind = 8 ) / ( dl(ilo) + dr(jj) )
        do jc = 1, m
          aj(jj,jc) = fact * ( aj(jj+1,jc) - aj(jj,jc) )
        end do
        ilo = ilo - 1
      end do
...
!
    do j = jderiv+1, k-1
      ilo = k-j
      do jj = 1, k-j
        fact = 1.0D+00 / ( dl(ilo) + dr(jj) )
        do jc = 1, m
          aj(jj,jc) = ( aj(jj+1,jc) * dl(ilo) + aj(jj,jc) * dr(jj) ) &
                    * fact
        end do
        ilo = ilo - 1
      end do
...
!
  ...
  integer ( kind = 4 ), intent(in) :: npoint, ncol

  integer ( kind = 4 ) i
  real ( kind = 8 ), intent(in) :: p, qty(npoint)
  real ( kind = 8 ), intent(out) :: qu(npoint), u(npoint)
  real ( kind = 8 ), intent(inout) :: v(npoint,7)
  real ( kind = 8 ) prev
  real ( kind = 8 ) ratio
  real ( kind = 8 ) six1mp
  real ( kind = 8 ) twop
...
!
  ...
  real ( kind = 8 ) a(ndim)
  real ( kind = 8 ), intent(in) :: aleft, aright, addbrk, relerr
  real ( kind = 8 ) amax
  ...
  integer ( kind = 4 ) iter
  integer ( kind = 4 ), intent(in) :: lbegin, iorder, ntimes
  integer ( kind = 4 ) lnew
  integer ( kind = 4 ) n
...
!
    do iter = 1, itermx
      ...
      call bsplpp ( t, a, n, kpm, templ, breaks, coef, l )
...
!
    ...
    do i = 1, l
      ii = ( i - 1 ) * kpm
      write ( *, '(f9.3,2x,e12.4,10e11.3)' ) breaks(i), coef(ii+1:ii+kpm)
    end do
!
...
!
    ...
    call newnot ( breaks, coef, l, kpm, temps, lnew, templ )
...
!
  ...
  real ( kind = 8 ), intent(inout) :: c(4,n)
  real ( kind = 8 ) divdf1
  real ( kind = 8 ) divdf3
  real ( kind = 8 ) dtau
  real ( kind = 8 ) g
...
!
  if ( ibcbeg == 0 ) then
    ...
    if ( n <= 2 ) then
      ...
      c(4,1) = c(3,3)
      c(3,1) = c(3,2) + c(3,3)
      c(2,1) = ( ( c(3,2) + 2.0D+00 * c(3,1) ) * c(4,2) &
...
!
    ...
    write ( *, '(/,a)' )'CUBSPL - Error, invalid IBCBEG input option!'
    stop 1
  end if
...
!
!  If slope is prescribed at right end ( ibcend == 1 ), one can go
!  directly to back-substitution, since the C array happens to be
!  set up just right for it at this point.
!
    if ( ibcend < 1 ) then
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: nbloks, ncols, nequ

  real ( kind = 8 ) awi1od
  real ( kind = 8 ), intent(inout) :: b(nequ)
  real ( kind = 8 ) colmax
  integer ( kind = 4 ) i
  ...
  real ( kind = 8 ), intent(out) :: x(nequ), d(nequ)
...
!
    ...
    un = ppvalu ( breaks, coef, l, kpm, xx, 0 )
...
!
    ...
    do i = 1, 9
      ...
      value = ppvalu ( breaks, coef, l, kpm, x, 0 )
...
!
  ...
  real ( kind = 8 ), intent(out) :: detlog, detsgn
  integer ( kind = 4 ) i
  ...
  integer ( kind = 4 ) ip
  integer ( kind = 4 ), intent(in) :: ipivot(1)
  integer ( kind = 4 ) k
  integer ( kind = 4 ) last
  integer ( kind = 4 ) nrow
...
!
  ...
  integer ( kind = 4 ), intent(in) :: kpm, n

  real ( kind = 8 ), intent(out) :: b(*)
  real ( kind = 8 ), intent(out) :: bloks(*)
  integer ( kind = 4 ) i
  integer ( kind = 4 ) index
  integer ( kind = 4 ) indexb
  integer ( kind = 4 ), intent(out) :: integs(3,*)
  integer ( kind = 4 ) isidel
  integer ( kind = 4 ) left
  integer ( kind = 4 ), intent(in) :: lenblk
  integer ( kind = 4 ), intent(out) :: nbloks
  integer ( kind = 4 ) nrow
  real ( kind = 8 ), intent(in) :: t(n+kpm)
  real ( kind = 8 ), intent(out) :: work1(kpm,kpm)
  real ( kind = 8 ), intent(out) :: work2(kpm,*)
...
!
      ...
      do while (( isidel /= m ) .and. ( t(left+1) > xside(isidel+1) ))
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, lnew, l
  real ( kind = 8 ), intent(in) :: breaks(l+1), coef(k,l)
  real ( kind = 8 ), intent(out) :: brknew(lnew+1), coefg(2,l)
  integer ( kind = 4 ) i
  ...
  if ( lnew == 0 ) then
    ...
    brknew(1) = 0.5D+00 * ( breaks(1) + breaks(l+1) )

  else

    do i = 1, lnew + 1
      brknew(i) = ( real ( lnew - i + 1, kind = 8 ) * breaks(1) &
                  + real (        i - 1, kind = 8 ) * breaks(l+1) ) &
                  / real ( lnew,         kind = 8 )
    end do
...
!
  ...
  integer ( kind = 4 ), intent(in) :: ncol, nrow, last

  real ( kind = 8 ) awikdi
  real ( kind = 8 ) colmax
  real ( kind = 8 ), intent(out) :: d(nrow)
  integer ( kind = 4 ) i
  integer ( kind = 4 ), intent(inout) :: iflag
  integer ( kind = 4 ) ipivi
  integer ( kind = 4 ) ipivk
  integer ( kind = 4 ), intent(out) :: ipivot(nrow)
  integer ( kind = 4 ) j
  integer ( kind = 4 ) k
  integer ( kind = 4 ) kp1
  real ( kind = 8 ) ratio
  real ( kind = 8 ), intent(inout) :: w(nrow,ncol)
!
...
!
  ...
  d = maxval( abs(w), dim=2 )
  if ( any( d == 0.0D+00 ) ) then
    iflag = 0
    return
  end if
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: integs(3,nbloks)
  integer ( kind = 4 ), intent(out) :: ipivot(*)
  integer ( kind = 4 ) last
  integer ( kind = 4 ) ncol
  integer ( kind = 4 ) nrow
  real ( kind = 8 ), intent(out) :: scrtch(*)
...
!
  ...
  nrow = integs(1,nbloks)
  ncol = integs(2,nbloks)
  last = integs(3,nbloks)
!
!  Carry out elimination on the (nbloks)-th block, for columns 1
!  through LAST of I-th block.
!
  call factrb ( bloks(indexn), ipivot(indexb), scrtch, nrow, ncol, &
    last, iflag )
...
!
  mflag = transfer( xt(lxt) < x, mflag)
  ihi = ilo + 1

  if ( lxt <= ilo ) then

    if ( xt(lxt) <= x ) then
      ilo = minloc( xt, dim= 1, mask= xt >= xt(lxt) ) - 1
      left = ilo
      return
    end if
...
!
  ...
  integer ( kind = 4 ), intent(in) :: kpm, l, m
  integer ( kind = 4 ), intent(out) :: n

  real ( kind = 8 ), intent(in) :: breaks(l+1)
  integer ( kind = 4 ) j
  integer ( kind = 4 ) jj
  integer ( kind = 4 ) lp1
  integer ( kind = 4 ) k

  real ( kind = 8 ), intent(out) :: t(l*kpm+kpm-l*m+m) ! t(n+kpm)

  k = kpm - m
  n = l * k + m
  lp1 = l + 1
  
  t(n+1:n+kpm) = breaks(lp1)
  
  do j = 1, l
    jj = n - k * j
    t(jj+1:jj+k) = breaks(lp1-j)
  end do
...
!
  ...
  real ( kind = 8 ), intent(out) :: bcoef(n)
  real ( kind = 8 ), intent(out) :: diag(n)
  real ( kind = 8 ), intent(out) :: q(k,n)
  real ( kind = 8 ) biatx(k)
  real ( kind = 8 ) dw
  integer ( kind = 4 ) i
  integer ( kind = 4 ) j
...
!
    do while (( left /= n) .and. ( tau(ll) >= t(left+1) ))
...
subroutine l2err ( iprfun, breaks, coef, l, k, ntau, tau, gtau, &
                & weight, ftau, error )
!
!*****************************************************************************80
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: iprfun, ntau, l, k

  real ( kind = 8 ), intent(in) :: tau(ntau), gtau(ntau), weight(ntau)
  real ( kind = 8 ), intent(in) :: breaks(l+1), coef(k,l)
  real ( kind = 8 ), intent(out) :: ftau(ntau), error(ntau)
  real ( kind = 8 ) err
  real ( kind = 8 ) errl1
  real ( kind = 8 ) errl2
  real ( kind = 8 ) errmax
  ...
  do ll = 1, ntau
    ...
    ftau(ll) = ppvalu ( breaks, coef, l, k, tau(ll), 0 )
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, l
  integer ( kind = 4 ), intent(out) :: n

  real ( kind = 8 ), intent(in) :: breaks(l+1)
  real ( kind = 8 ), intent(out) :: t(k-1+l+k)

  n = k - 1 + l

  t(1:k-1) = breaks(1)
  t(k:n) = breaks(1:l) 
  t(n+1:n+k) = breaks(l+1)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: k, l, lnew
...
!
  if ( l <= 1 ) then
    call evnnot ( breaks, coef, l, k, brknew, lnew, coefg )
    return
  end if

  brknew(1) = breaks(1)
  brknew(lnew+1) = breaks(l+1)
!
!  Construct the continuous piecewise linear function G.
!
  oneovk = 1.0D+00 / real ( k, kind = 8 )
  coefg(1,1) = 0.0D+00
  difprv = abs ( coef(k,2) - coef(k,1) ) / ( breaks(3) - breaks(1) )
  
  do i = 2, l
    dif = abs ( coef(k,i) - coef(k,i-1) ) / ( breaks(i+1) - breaks(i-1) )
    coefg(2,i-1) = ( dif + difprv )**oneovk
    coefg(1,i) = coefg(1,i-1) + coefg(2,i-1) * ( breaks(i) - breaks(i-1) )
    difprv = dif
  end do
...
!  STEP = G(B) / LNEW.
!
  step = ( coefg(1,l) + coefg(2,l) * ( breaks(l+1) - breaks(l) ) ) &
    / real ( lnew, kind = 8 )
...
!
!    G(BREAKS(J)) <= STEPI <= G(BREAKS(J+1))
!
!  and then
!
!    BRKNEW(I) = BREAKS(J) + ( STEPI - G(BREAKS(J)) ) / DG(BREAKS(J) ).
!
!  The midpoint is chosen if DG(BREAKS(J)) = 0.
!
  j = 1
...
!
!    end do
    j = count(stepi > coefg(1,1:l))
       
    if ( coefg(2,j) /= 0.0D+00 ) then
      brknew(i) = breaks(j) + ( stepi - coefg(1,j) ) / coefg(2,j)
    else
      brknew(i) = ( breaks(j) + breaks(j+1) ) / 2.0D+00
    end if
...
!
    call interv ( breaks, l+1, x, i, ndummy )
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: kpm, left, nrow

  real ( kind = 8 ), intent(out) :: b(nrow)
  real ( kind = 8 ), intent(inout) :: dbiatx(kpm,*)
  real ( kind = 8 ) dx
  integer ( kind = 4 ) irow
  integer ( kind = 4 ) j
  integer ( kind = 4 ) ll
  ...
  integer ( kind = 4 ) mode
  integer ( kind = 4 ) mp1
  real ( kind = 8 ), intent(out) :: q(nrow,kpm)
  real ( kind = 8 ), intent(out) :: scrtch(kpm,kpm)
  real ( kind = 8 ), intent(in) :: t(left+kpm)
  real ( kind = 8 ) v(20)
  real ( kind = 8 ) xm
  real ( kind = 8 ) xx
  ...
  do irow = lowrow, nrow
    ...
    if (( k >= ll ) .and. (( m < iside ) .or. ( xx < xside(iside) )) ) then
!
...
!
  ...
  integer ( kind = 4 ), intent(in) :: integs(3,nbloks)
  integer ( kind = 4 ), intent(in) :: ipivot(*)
  integer ( kind = 4 ) j
  integer ( kind = 4 ) last
  integer ( kind = 4 ) nbp1
  integer ( kind = 4 ) ncol
...
!
  ...
  real ( kind = 8 ) diff
  real ( kind = 8 ), intent(in) :: dx(npoint)
  integer ( kind = 4 ) i
  real ( kind = 8 ) prev
  real ( kind = 8 ), intent(out) :: qty(npoint), v(npoint,7)
  real ( kind = 8 ), intent(in) :: x(npoint), y(npoint)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: ncoli, ncoli1, nrowi1, nrowi, last

  real ( kind = 8 ), intent(in) :: ai(nrowi,ncoli)
  real ( kind = 8 ), intent(inout) :: ai1(nrowi1,ncoli1)
  integer ( kind = 4 ) ip
  integer ( kind = 4 ), intent(in) :: ipivot(nrowi)
  integer ( kind = 4 ) j
  integer ( kind = 4 ) m

  if (( nrowi - last >= 1 ) .and. ( ncoli - last >= 1 )) then
!
...
!
  ...
  real ( kind = 8 ), intent(out) :: a(npoint,4)
  real ( kind = 8 ) change
  real ( kind = 8 ), intent(in) :: dy(npoint)
  integer ( kind = 4 ) i
  real ( kind = 8 ) oosf
  real ( kind = 8 ) ooss
  real ( kind = 8 ) p
  ...
  real ( kind = 8 ) utru
  real ( kind = 8 ), intent(out) :: v(npoint,7)
  real ( kind = 8 ), intent(in) :: x(npoint), y(npoint)
...
!
      ...
      do
        ...
        sfq = dot_product ( a(1:npoint,1)**2, dy(1:npoint)**2 )
        sfq = 36.0D+00 * sfq / ( 1.0D+00 + q )**2
...
!f2py real ( kind = 8 ), intent(in), dimension(*,*) :: gtau
  ...
  real ( kind = 8 ), intent(in) :: tau(n), t(n+k)
  integer ( kind = 4 ) i
  integer ( kind = 4 ), intent(out) :: iflag
  ...
  integer ( kind = 4 ) left
  real ( kind = 8 ) taui
  real ( kind = 8 ), intent(out) :: q((2*k-1)*n)
  real ( kind = 8 ), intent(out) :: work(n)
  real ( kind = 8 ), intent(out) :: bcoef(m,n)
...
!
    ...
    do while (( t(left+1) <= taui ).and.( left+1 < ilp1mx ))
...
!
  ...
  real ( kind = 8 ), intent(out) :: bcoef(n)
  real ( kind = 8 ), intent(in) :: tau(n), gtau(n), t(n+k)
  integer ( kind = 4 ) i
  integer ( kind = 4 ), intent(out) :: iflag
  integer ( kind = 4 ) ilp1mx
  integer ( kind = 4 ) j
...
!
  ...
  integer ( kind = 4 ) nmk
  integer ( kind = 4 ) nx
  real ( kind = 8 ), intent(out) :: scrtch((n-k)*(2*k+3)+5*k+3)
  real ( kind = 8 ), intent(out) :: t(n+k)
  real ( kind = 8 ), intent(in) :: tau(n)
  real ( kind = 8 ) sign
  real ( kind = 8 ) signst
  real ( kind = 8 ) tol
...
!
  do newton = 1, newtmx
    ...
    do j = 1, nmk
      ...
      do while (( xij >= scrtch(left+1) ) .and. (kpn > left + 1))
...
!
  ...
  integer ( kind = 4 ) ip
  integer ( kind = 4 ), intent(in) :: ipivot(nrow)
  integer ( kind = 4 ) k
  real ( kind = 8 ), intent(in) :: w(nrow,ncol)
  real ( kind = 8 ), intent(inout) :: x(ncol)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: last, nrow

  real ( kind = 8 ), intent(inout) :: b(nrow+nrow-last)
  integer ( kind = 4 ) ip
  integer ( kind = 4 ), intent(in) :: ipivot(nrow)
  integer ( kind = 4 ) jhi
  integer ( kind = 4 ) k
  real ( kind = 8 ), intent(in) :: w(nrow,last)
  real ( kind = 8 ), intent(out) :: x(nrow)
...
!
  ...
  integer ( kind = 4 ), intent(in) :: ntau

  real ( kind = 8 ) alph
  real ( kind = 8 ) alpha
  real ( kind = 8 ), intent(out) :: breaks(*)
  real ( kind = 8 ) c
  real ( kind = 8 ), intent(out) :: coef(4,*)
  real ( kind = 8 ) d
  real ( kind = 8 ) del
  real ( kind = 8 ) denom
  real ( kind = 8 ) divdif
  ...
  real ( kind = 8 ), intent(in) :: gtau(ntau)
  integer ( kind = 4 ) i
  integer ( kind = 4 ), intent(out) :: iflag
  integer ( kind = 4 ), intent(out) :: k, l
  integer ( kind = 4 ) method
  real ( kind = 8 ) onemg3
  real ( kind = 8 ) onemzt
  real ( kind = 8 ) ratio
  real ( kind = 8 ), intent(out) :: s(ntau,6)
  real ( kind = 8 ) :: sixth, third
  real ( kind = 8 ), intent(in) :: tau(ntau)
  real ( kind = 8 ) x
  real ( kind = 8 ) z
  real ( kind = 8 ) zeta
  real ( kind = 8 ) zt2
...
!
    ...
    if ( ( method == 3 ) .or. &
         (( method /= 1 ) .and. &
          ( s(i,4) * s(i+1,4) >= 0.0D+00 )) ) then

      denom = abs ( s(i,4) ) + abs ( s(i+1,4) )
...
!
    if ( z < 0.5D+00 ) then
      ...
      s(i,6) = zeta * factor * sixth
      s(i,2) = s(i,2) + s(i,1) &
        * ( ( 1.0D+00 - alpha * onemzt ) * factor * 0.5D+00 - s(i,6) )
!
...
!
    ...
    if ( z < 0.5D+00 ) then
      ...
      ratio = - ( s(i,1) * sixth ) / s(i,2)
      s(i+1,2) = s(i,1) &
      * ( ( 1.0D+00 - zeta * alpha ) * factor * 0.5D+00 - s(i,6) )
...
!
  entry1 = ratio * s(i-1,3) + s(i,2) + s(i,1) * third
  s(i+1,2) = s(i,1) * sixth
  ...
  do i = 1, ntau - 1
    ...
    if ( z == 0.0D+00 ) then
      ...
      coef(2,l-1) = coef(2,l) - del * ( coef(3,l) &
        - ( del * 0.5D+00 ) * coef(4,l-1))
 
    else if ( z == 0.5D+00 ) then

      coef(2,l) = divdif &
        - s(i,1) * ( 2.0D+00 * s(i,4) + s(i+1,4) ) * sixth
      ...
      if ( onemzt == 0.0D+00 ) then
        ...
        coef(1,l) = coef(1,l-1) + del * ( coef(2,l-1) &
          + ( del * 0.5D+00 ) * ( coef(3,l-1) &
          + ( del * third ) * coef(4,l-1) ) )
...
!-----------------------------------------------------------------------------80
! numerical library for polynomials in Newton, from:
!http://people.sc.fsu.edu/~jburkardt/f_src/newton_interp_1d/newton_interp_1d.f90
...
!f2py integer, depend(f) :: n = shape(f,0)
!f2py integer, check(n*m == shape(f,0)*shape(f,1)), depend(f,n) :: m = shape(f,1)
  integer ( kind = 4 ), intent(in) :: n, m

!f2py real ( kind = 8 ), intent(in), dimension(*,*) :: f
  real ( kind = 8 ), intent(in) :: f(n,m)
  real ( kind = 8 ), intent(in) :: x(n)
  real ( kind = 8 ), intent(out) :: d(m,n)
...
!
  do i = 2, n
    do j = n, i, -1
      ...
      diff = 1.0D+00 / ( x(j) - x(j+1-i) )
      
      do k = 1, m
        
        d(k,j) = ( d(k,j) - d(k,j-1) ) * diff
...
!
  ...
  integer ( kind = 4 ), intent(in) :: n, ni

  real ( kind = 8 ), intent(in) :: d(n), x(n), xi(ni)
  real ( kind = 8 ), intent(out) :: fi(ni)
  integer ( kind = 4 ) i

  fi(1:ni) = d(n)
  do i = n - 1, 1, -1
    fi(1:ni) = d(i) + ( xi(1:ni) - x(i) ) * fi(1:ni)
  end do
...
!f2py integer, depend(d) :: n = shape(d,0)
!f2py integer, check(n*m == shape(d,0)*shape(d,1)), depend(d,n) :: m = shape(d,1)
  integer ( kind = 4 ), intent(in) :: n, m

!f2py real ( kind = 8 ), intent(in), dimension(*,*) :: d
  real ( kind = 8 ), intent(in) :: d(n,m)
  real ( kind = 8 ), intent(in) :: x(n), xi
  real ( kind = 8 ), intent(out) :: fi(m)
  integer ( kind = 4 ) :: i, j
  real ( kind = 8 ) fact
...
!
  ...
  real ( kind = 8 ), intent(in) :: a1(n), a2(n)
  integer ( kind = 4 ) i
  character ( len = * ), intent(in) :: title
