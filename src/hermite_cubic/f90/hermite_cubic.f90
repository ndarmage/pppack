! library from:
! https://people.sc.fsu.edu/~jburkardt/f_src/hermite_cubic/hermite_cubic.f90
...
!
  ...
  real ( kind = 8 ), intent(out) :: d(4,n)
  real ( kind = 8 ) dx(n)
  real ( kind = 8 ), intent(out) :: f(4,n)
  real ( kind = 8 ) h
  real ( kind = 8 ), intent(out) :: s(4,n)
  real ( kind = 8 ), intent(out) :: t(4,n)
...
!
  ...
  real ( kind = 8 ), intent(in) :: dn(nn)
  real ( kind = 8 ), intent(in) :: fn(nn)
  real ( kind = 8 ), intent(out) :: q
  real ( kind = 8 ), intent(in) :: xn(nn)
